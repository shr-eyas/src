#include <fr3_controllers/joint_impedance_controller.h>
...
/*-------------------------------PUT YOUR LOGIC HERE------------------------------*/
  ...
  // RETRIVE CORIOLIS FORCE VECTOR 
  std::array<double, 7> coriolis = model_handle_->getCoriolis();
  // RETRIVE GRAVITY VECTOR
  ...
  // COMPUTE THE COMMAND LAW
  // 'k_gains_[i]' is the proportional gain for joint i.
  // 'robot_state.q_d[i]' is the desired position of joint i.
  // 'robot_state.q[i]' is the actual measured position of joint i.

  // 'd_gains_[i]' is the derivative gain for joint i.
  // 'robot_state.dq_d[i]' is the desired velocity of joint i.
  // 'dq_filtered_[i]' is the measured velocity of joint i.

  std::array<double, 7> tau_d_calculated;
  for (size_t i = 0; i < 7; ++i) {
    tau_d_calculated[i] = coriolis_factor_ * coriolis[i] + (k_gains_[i]*(robot_state.q_d[i]-robot_state.q[i])) + (d_gains_[i]*(robot_state.dq_d[i]-dq_filtered_[i])) ; /* ADD JOINT IMPEDANCE LAW HERE */
    }
